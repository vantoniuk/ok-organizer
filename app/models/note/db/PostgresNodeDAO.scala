...
class Nodes(tag: DBTag) extends Table[Node](tag, "nodes") {
  ...
  def completed = column[Option[DateTime]]("completed")
  def expired = column[Option[DateTime]]("expired")
  def service = column[ServiceId]("service")

  def * = (id, parentId, nodeType, title, description, icon, priority, rating, author, created, expired, completed, service) <> (Node.apply _ tupled, Node.unapply)
